...
begin
    ...
    {    for i:=1 to k do if boo[i] then begin
        for j:=1 to k do if boo[j] then begin
            giong:=true;
            for u:=2 to m do if a[i,u]<>a[j,u] then begin
                giong:=false;
                break;
            end;
            if giong then begin
                if i<j then boo[j]:=false;
                continue;
            end;
            boo[j]:=false;
            for u:=2 to m do if (a[i,u]=0)and(a[j,u]=1) then begin
                boo[j]:=true;
                break;
            end;
        end;
